        ...
        tabs.forEach((element) => {
            ...
            tabsBtn.forEach((el, index) => {
                ...
                content.insertAdjacentHTML(
                    'beforebegin',
                    '<div class="tabs__btn-accordion  btn btn-primary  mb-1 '
                        .concat(active, '" data-tab-btn="')
                        .concat(data, '">')
                        .concat(el.innerHTML, '</div>')
                );
            });
...
function eventHandler() {
    ...
    if (screenName && x.includes('localhost:30')) {
        document.body.insertAdjacentHTML(
            'beforeend',
            '<div class="pixel-perfect" style="background-image: url(screen/'.concat(screenName, ');"></div>')
        );
    }
